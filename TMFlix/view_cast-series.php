...
$ID = $_GET['identity'];
$sql = "SELECT Cast_Name, Cast_ID FROM cast WHERE Cast_ID='$ID';";
...
<html>
<head>
		<title><?php echo $name;?>- Cast-Series</title>
...
<?php	
include "connection.php";
$sql = "SELECT series_cast.Series_ID,series.Series_Name,series.Series_Poster
FROM series_cast 
INNER JOIN series
ON series.Series_ID = series_cast.Series_ID
INNER JOIN cast
ON cast.Cast_ID = series_cast.Cast_id
WHERE series_cast.Cast_id='$ID'";
$result = $conn->query($sql);
 for($i=0; $row = $result->fetch_assoc(); $i++){
