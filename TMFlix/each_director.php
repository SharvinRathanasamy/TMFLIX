...
<html>
<head>
		<title><?php echo $seriesname;?>- Director</title>
...
<div id="body">
<h3><?php echo $seriesname;?>- Director</h3>
...
<?php	
include "connection.php";
$sql = "SELECT series_director.Series_ID,director.Director_Name,series_director.Director_ID,series.Series_Name
FROM series_director 
INNER JOIN series
ON series.Series_ID = series_director.Series_ID
INNER JOIN director
ON director.Director_ID= series_director.Director_ID
WHERE series_director.Series_ID='$seriesid'";
$result = $conn->query($sql);
 for($i=0; $row = $result->fetch_assoc(); $i++){
...
?>					
			...
			<tr>
				...
				<td><br>
				<form method="post">
				<input type='hidden' name='deletedirid' value='<?php echo $row['Director_ID']; ?>'>
...
if($g_dirid==-1)
{
    ...
    echo '<p>Genre is empty</p>';
			...
			$sql2 = "SELECT * FROM series_director WHERE Series_ID='$g_seriesid' AND Director_ID='$g_dirid' ";
			...
			$sql3 = "INSERT into series_director (Series_ID,Director_ID) 
						VALUES ('$g_seriesid','$g_dirid')"; 
...
$ddirid=$_POST["deletedirid"];
$sql5="DELETE FROM series_director WHERE Series_ID='$dsiid' AND Director_ID='$ddirid' ";
