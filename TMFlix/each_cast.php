...
<html>
<head>
		<title><?php echo $seriesname;?>- Cast</title>
...
<div id="body">
<h3><?php echo $seriesname;?>- Cast</h3>
...
<?php	
include "connection.php";
$sql = "SELECT series_cast.Series_ID,cast.Cast_Name,series_cast.Cast_id,series.Series_Name
FROM series_cast 
INNER JOIN series
ON series.Series_ID = series_cast.Series_ID
INNER JOIN cast
ON cast.Cast_ID = series_cast.Cast_id
...
?>					
			...
			<tr>
				...
				<td><br>
				<form method="post">
				<input type='hidden' name='deletecastid' value='<?php echo $row['Cast_id']; ?>'>
...
<input type="text" disabled="disabled"  name="seriesnamea" value="<?php echo $seriesname;?>" required>
			...
			<select name="cast" id="cast" name="cast" required>
				<option value="-1" selected>Cast</option>
<?php	
...
if($g_castid==-1)
{
    ...
    echo '<p>Genre is empty</p>';
			...
			$sql2 = "SELECT * FROM series_cast WHERE Series_ID='$g_seriesid' AND Cast_id='$g_castid' ";
			...
			$sql3 = "INSERT into series_cast (Series_ID,Cast_id) 
						VALUES ('$g_seriesid','$g_castid')"; 
...
$dcastid=$_POST["deletecastid"];
$sql5="DELETE FROM series_cast WHERE Series_ID='$dsiid' AND Cast_id='$dcastid' ";
